    ...
    <>
      ...
      <aside
        className={` shrink-0 border-r border-gray-200 bg-white max-h-screen fixed top-0 left-0 bottom-0 z-20 transition-all duration-300 ${
          isCollapsed
            ? "md:w-[80px] w-[286px] -translate-x-full md:translate-x-0"
            : "md:w-[286px] "
        }`}
      >
        <Button
          onClick={onToggle}
          className="absolute hidden! md:flex! p-0! top-[29px] right-0 h-6! w-6! rounded-md border border-primary/40 text-primary! hover:text-white! items-center justify-center translate-x-1/2 bg-white shadow-[0px_4px_4px_0px_#40A19840]"
        >
          <div
        ...
        >
          <Link to="/" className="inline-block">
            <img
              src={isSmallScreen ? Logo : isCollapsed ? ShortLogo : Logo}
              alt="Letmegrab"
            />
          </Link>
          <nav className="overflow-y-auto max-h-[calc(100vh-210px)] custom-scrollbar mt-[30px] space-y-8 md:space-y-14">
            <div>
              <h6
                className={`text-sm font-medium text-gray px-4 mb-2.5 whitespace-nowrap ${
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    <li key={item.path} className="overflow-hidden">
                      <Link
                        to={item.path}
                        className={`flex items-center gap-3.5 font-medium  rounded-[10px] text-base md:text-lg leading-normal transition-all duration-300 ${
                          isActive
                ...
                <li>
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={toggleTheme}
                      className={`relative rounded-full bg-background transition-all duration-300 flex p-1 justify-between cursor-pointer ${
                        isCollapsed
                          ? "w-[68px] h-[34px] md:w-[34px] md:h-[68px] flex-row md:flex-col ml-auto md:mx-auto"
                          : "w-[68px] h-[34px]  ml-auto"
                      }`}
